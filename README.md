...
game-auto-test/
├── config/              # 配置层
│   ├── settings.py      # 环境地址、账号、超时、消息 ID 常量
│   └── loader.py        # 配表加载（从 SQL/Excel 解析游戏配表）
├── client/
...
3. **Protobuf 鉴权**：构造 C2S_AUTH 消息，填 cuid / rid / ts / token / lang / currency，封包后发出去。服务端回 S2C_AUTH，成功判定标准是 server_ts > 0（服务端给当前时间戳就说明验明正身了）。

4. **心跳保活**：启动 daemon 后台线程，每隔固定间隔发一条 C2S_HEARTBEAT 空消息，告诉服务端"我还活着"，防止服务端因为一段时间没消息主动断开连接。
...
- max_retry：服务端疯狂推无关消息时，最多收这么多帧真实消息就放弃，防止推送风暴把程序拖死
- total_timeout：服务端一直沉默时，最多等这么多秒就放弃，防止无限干等。超时帧不计入 max_retry，因为沉默不是风暴
